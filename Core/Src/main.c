  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  uint32_t adc_val = 1;
	      ...
	      // 2. Poll for conversion (wait up to 10ms)
	      if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK)
	      {
	          // 3. Get the digital value (0 to 4095)
	          adc_val = HAL_ADC_GetValue(&hadc1);

	          // 4. Threshold Logic: 1.65V is approx 2047
	          if (adc_val > 2047)
	          {
	              // Voltage is HIGH (> 1.65V): Green ON, Red OFF
	              HAL_GPIO_WritePin(GPIOF, LED_GREEN_Pin, GPIO_PIN_SET);
	              HAL_GPIO_WritePin(GPIOF, LED_RED_Pin, GPIO_PIN_RESET);
	          }
	          else
	          {
	              // Voltage is LOW (< 1.65V): Red ON, Green OFF
	              HAL_GPIO_WritePin(GPIOF, LED_GREEN_Pin, GPIO_PIN_RESET);
	              HAL_GPIO_WritePin(GPIOF, LED_RED_Pin, GPIO_PIN_SET);
	          }
	      }

	      // 5. Stop ADC to save power/reset for next cycle
	      HAL_ADC_Stop(&hadc1);

	      // Small delay to make the LED transition look smooth
	      HAL_Delay(20);
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
